...
$hero_wrapper_attrs = array(
	...
	'aria-label'           => __( 'Hero slideshow', 'aggressive-blocks' ),
	'style'                => implode( ' ', $hero_style_parts ),
	'data-wp-interactive'  => 'aggressive-blocks/hero-carousel',
	'data-wp-context'      => (string) wp_json_encode(
		array(
			...
			'i18n'          => array(
				'play'  => __( 'Play slideshow', 'aggressive-blocks' ),
				'pause' => __( 'Pause slideshow', 'aggressive-blocks' ),
				/* translators: 1: current slide number, 2: total slide count. Announced by screen readers. */
				'slide' => __( 'Slide %1$s of %2$s', 'aggressive-blocks' ),
			),
		)
	),
		...
		<div class="aa-hero__track">
			<?php foreach ( $hero_slides as $hero_index => $hero_slide_block ) : ?>
				...
				<div
					...
					aria-label="<?php echo esc_attr( sprintf( __( '%1$s of %2$s', 'aggressive-blocks' ), $hero_index + 1, $hero_count ) ); ?>"
				...
				>
					<?php echo aggressive_blocks_trusted_html( $hero_slide_html ); ?>
				</div>
			<?php endforeach; ?>
		</div>
		...
		<?php if ( ( 'none' !== $hero_pagination || $hero_autoplay ) && $hero_count > 1 ) : ?>
			<div class="aa-hero__bar">
				...
				<?php elseif ( 'none' !== $hero_pagination ) : ?>
					<div class="aa-hero__pagination aa-hero__pagination--<?php echo esc_attr( $hero_pagination ); ?><?php echo $hero_show_progress && $hero_autoplay ? ' aa-hero__pagination--progress' : ''; ?>" role="group" aria-label="<?php esc_attr_e( 'Choose slide', 'aggressive-blocks' ); ?>">
						<?php for ( $hero_dot = 0; $hero_dot < $hero_count; $hero_dot++ ) : ?>
							<button
								type="button"
								class="aa-hero__dot"
								<?php /* translators: %s: slide number. */ ?>
								aria-label="<?php echo esc_attr( sprintf( __( 'Go to slide %s', 'aggressive-blocks' ), $hero_dot + 1 ) ); ?>"
							...
							>
								...
								<?php elseif ( 'thumbnails' === $hero_pagination ) : ?>
									<?php echo aggressive_blocks_trusted_html( $hero_slide_thumb( $hero_slides[ $hero_dot ]['attrs'] ?? array() ) ); ?>
								<?php endif; ?>
							</button>
						<?php endfor; ?>
					</div>
				...
				<?php if ( $hero_autoplay ) : ?>
					<button
						...
						aria-label="<?php esc_attr_e( 'Pause slideshow', 'aggressive-blocks' ); ?>"
